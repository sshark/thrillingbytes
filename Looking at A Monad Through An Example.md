...
> **Sidebar**\
> A Monad, a typeclass[^tc], has the combined `map` and `flatMap` methods where `map` comes from the Functor.  A Monad is a subclass of *Applicative* which in turn a subclass of *Functor*.
> 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTAxODQwNzA0NCwyMDc4NDQwODQsLTE0OD
